...
var G = {
			...
			if (!str) {
				...
				if (ms) {
					for ( var i = 0; i < ms.length; i++) {
						var x = ms[i].match(p2);
						if (x.length > 1) {
							get[x[0]] = decodeURIComponent(x[1]);
						}
					}
				}
				return get;
	...
	util : {
		...
		documentTop : function() {
			return (G.nav.isIE ? document.documentElement.scrollTop
	...
	valid : {
		...
		float : function(e) {
			var expreg = new RegExp("^[\\+\\-\\d" + G.valid.decimalPoint + "]$");
		...
		isFloat : function(t, p) {
			if (!p) {
				p = "\\.,";
			}
			var expreg = new RegExp("^[\\+\\-]{0,1}\\d*" + p + "{0,1}\\d+$")
			return G.valid.validar(t, expreg);
		},
		isEmail : function(t) {
			return G.valid
					.validar(t,
					...
					if (!d) {
						...
						if (this.capa.id) {
							this.timeHander = setTimeout(
									"G.Desplazantes.search('" + this.idcapa
											+ "').animar();", tiempo);
						}
					}
				}
	...
	OwnTextBox : {
		create : function(id, name, value, textodefecto) {
			...
			if (!textodefecto) {
				textodefecto = G.dom.data(otb, "text");
			}
		...
		convert : function(el, textodefecto) {
			...
			if (el) {
				if (!textodefecto) {
					textodefecto = G.dom.data(el, "text");
				}
				el.classDefecto = G.dom.data(el, "class");
				if (!G.nav.isIE
						|| (G.nav.isIE && G.nav.version >= 9)
						|| (G.nav.isIE && G.nav.version < 9 && el.type == "text")) {
					el.originalType = el.type;
					...
					G.event.addEvent(el, "blur", function() {
						if (G.util.trim(this.value) === "") {
							...
							if (this.classDefecto) {
								this.className += " " + this.classDefecto;
							}
						}
					});
					G.event.addEvent(el, "change", function() {
						if (G.util.trim(this.value) !== this.defaultText) {
							this.type = this.originalType;
						} else {
							this.type = "text";
						}
	...
	autocompletar : {
		...
		esconder : function() {
			this.timer = setTimeout("G.autocompletar.lista['"
					+ this.getAttribute("nombre")
					+ "'].capaBusqueda.style.display='none'", 1000);
		},
		mostrar : function() {
